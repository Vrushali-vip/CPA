        ...
        <div className="bg-white/90 p-6 md:p-8 shadow-lg shadow-[#1A2C50] rounded-md">
          <form onSubmit={form.handleSubmit(onSubmitForm)} className="space-y-8">
              ...
              <>
                ...
                <div className="mt-8 pt-6 border-t">
                  ...
                  {cityFields.map((field, index) => {
                    ...
                    return (<div key={field.id} className="flex items-end gap-2 mb-3"><div className="flex-grow"><InputWithLabel label={t(`insuranceForm.step2.cityName${index + 1}`)} name={cityNamePath} register={register} error={getError(cityNamePath)} placeholder={t("insuranceForm.step2.enterCityName")}/><input type="hidden" {...register(cityZoneTypePath)} value="GREEN"/></div><Button type="button" variant="destructive" size="sm" onClick={() => removeCity(index)}>{t("insuranceForm.step2.remove")}</Button></div>);
                  })}
            ...
            <div className="flex flex-col sm:flex-row justify-between pt-8 mt-8 border-t gap-4">
              ...
              {/* If modifyChoicesAlert is a simple string, no need for returnObjects: true */}
