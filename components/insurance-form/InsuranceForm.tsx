...
//     // 1. Validate partFieldsToValidate (if any for the current step)
//     const partFieldsToValidate: Path<InsuranceFormValues>[] = [];
//     if (step === 1) {
//       partFieldsToValidate.push(
//         "c_phone_code", "c_phone_number", "c_whats_app_code", "c_whats_app_number",
...
//     // 2. Validate currentStepFields (main fields for the step)
//     //    Only proceed if partValidation was okay (or not applicable).
...
//         // if schema is correctly set up.
//       }
//     }

//     // 4. If any validation failed, find and scroll to the first error, then return.
//     if (!overallValidationPassed) {
//       const fieldsToSearchForErrors = Array.from(new Set([...currentStepFields, ...partFieldsToValidate]));
//       let firstErrorKeyFound: Path<InsuranceFormValues> | undefined = fieldsToSearchForErrors.find(
//         (fieldName) => getError(fieldName) !== undefined
//       );
...
//               for (const Tfield of fieldsToCheck) {
//                 if (specificTravellerErrors[Tfield]) {
//                   firstErrorKeyFound = `travellers.${i}.${Tfield}` as Path<InsuranceFormValues>;
//                   break;
//                 }
//               }
//             }
...
//             const specificCityErrors = cityErrorsArray[i];
//             if (specificCityErrors?.name) {
//               firstErrorKeyFound = `trip_cities.${i}.name` as Path<InsuranceFormValues>;
//               break;
//             }
//           }
//         }
...
// // // ... (rest of interfaces and pricing, generateQuote function remain the same) ...
// // interface GenerateQuotePayload {
// //   trip_start_date: Date;
// //   trip_end_date: Date;
// //   trip_country: string;
...
// //   data?: { totalAmount: number; medicalCoverAmount: number; paCoverAmount: number; travellersCount: Integer; totalDays: number; };
// // }
// // function generateQuote(data: GenerateQuotePayload): QuoteResult { 
// //     const startDate = dayjs(data.trip_start_date).startOf('day');
// //     const endDate = dayjs(data.trip_end_date).endOf('day');
...
// //     if (totalDays <= 0) { return { ok: false, message: "Trip end date should be after trip start date.", warnings: [] }; }
// //     const sumOfZoneDays = data.green_zone_days + data.amber_zone_days + data.red_zone_days + data.black_zone_days;
// //     if (totalDays !== sumOfZoneDays) { return { ok: false, message: `Total trip days (${totalDays}) should be equal to sum of zone days (${sumOfZoneDays}).`, warnings: [] };}
// //     const medicalCoverageLevel = data.medical_coverage; const paCoverageLevel = data.pa_coverage;
// //     if ( (medicalCoverageLevel !== "0" && !pricing.matrix[medicalCoverageLevel]) || (paCoverageLevel !== "0" && !pricing.matrix[paCoverageLevel]) ) { return { ok: false, message: "No pricing found for selected non-zero coverage.", warnings: [] }; }
// //     const medicalRates = medicalCoverageLevel === "0" ? undefined : pricing.matrix[medicalCoverageLevel];
// //     const paRates = paCoverageLevel === "0" ? undefined : pricing.matrix[paCoverageLevel];
// //     let medicalCoverAmount = 0; let paCoverAmount = 0; const warnings: string[] = [];
// //     if (medicalRates && medicalCoverageLevel !== "0") { }
// //     if (paRates && paCoverageLevel !== "0") {   }
// //     let totalAmount = medicalCoverAmount + paCoverAmount;
// //     if (data.travellers_count > 0) totalAmount *= data.travellers_count;
// //     if (data.transit_coverage) totalAmount += pricing.transitCost;
// //     totalAmount = Math.round(totalAmount * 100) / 100;
// //     return { ok: true, message: "Quote generated successfully", warnings, data: { totalAmount, medicalCoverAmount: Math.round(medicalCoverAmount*100)/100, paCoverAmount: Math.round(paCoverAmount*100)/100, travellersCount: data.travellers_count, totalDays }};
// // }
...
// export default function InsuranceForm() {
//   const [showQuote, setShowQuote] = useState(false);
//   const { t } = useTranslation(); // For UI text
//   const { getValidationMessage } = useValidationMessages(); // For Joi messages

//   // Options that need dynamic translation from messages.json (validation part)
...
//   // Options that are static or might have labels translated differently (not via getValidationMessage)
//   // If their labels *were* in messages.json under 'options', you'd use getValidationMessage for them too.
...
//   // Regenerate the schema if the validationLocale changes
//   const schema = useMemo(() => getPurchaseWithoutLoginSchema(getValidationMessage), [getValidationMessage]);

//   const form = useForm<InsuranceFormValues>({
//     resolver: joiResolver(schema, { // Use the memoized schema
//       abortEarly: false,
//     }),
...
//       trip_countries: ["UA"],
//       travellers: [{ first_name: "", last_name: "", birthdate: "", passport_number: "", passport_expiry_date: "" }],
//       arrival_in_ukraine: "", departure_from_ukraine: "",
//       primary_cities_regions_ukraine: "", trip_cities: [], trip_purpose: "",
//       stay_name: "", company_name: "",
//       emergency_contact_first_name: "", emergency_contact_last_name: "",
...
//         keepValues: true,
//         // @ts-expect-error - joiResolver type might not perfectly match, but this is how you update it
//         resolver: joiResolver(getPurchaseWithoutLoginSchema(getValidationMessage), { abortEarly: false }),
//     });
...
//   const watchedPathsForQuote = ["trip_start_date", "trip_end_date", "trip_countries", "travellers", "emergency_medical_coverage", "personal_accident_coverage_level", "add_transit_coverage", "green_zone_days", "amber_zone_days", "red_zone_days", "black_zone_days"] as const;
//   const watchedValuesForQuote = watch(watchedPathsForQuote);

//   useEffect(() => {
//     const formValues = getValues();
//     const hasRequiredFieldsForQuote = formValues.trip_start_date && formValues.trip_end_date && formValues.trip_countries && formValues.trip_countries.length > 0 && formValues.trip_countries[0] && formValues.emergency_medical_coverage && formValues.personal_accident_coverage_level !== undefined && formValues.travellers && formValues.travellers.length > 0;
//     if (!hasRequiredFieldsForQuote) { setQuoteResult(null); return; }

//     const startDate = dayjs(formValues.trip_start_date + "T00:00:00");
//     const endDate = dayjs(formValues.trip_end_date + "T00:00:00");
//     if (!startDate.isValid() || !endDate.isValid() || endDate.isBefore(startDate)) { setQuoteResult({ ok: false, message: "Invalid trip dates for quote calculation.", warnings: [] }); return; }
    
//     // Use getValidationMessage for these checks now
//     const zoneDaysSumErrorCheck = getValidationMessage('insuranceForm.validationzoneDaysSum', {sumOfZoneDays:0, totalCalculatedDays:0}).substring(0,20);
//     const coverageOrErrorCheck = getValidationMessage('insuranceForm.validationcoverageOr').substring(0,20);

//     const criticalErrorFields: (Path<InsuranceFormValues>)[] = ["trip_start_date", "trip_end_date", "green_zone_days", "amber_zone_days", "red_zone_days", "trip_countries", "emergency_medical_coverage", "personal_accident_coverage_level"];
//     const hasVisibleFormErrors = criticalErrorFields.some(field => get(formState.errors, field)) || 
//                                  (formState.errors.root && (formState.errors.root.message?.includes(zoneDaysSumErrorCheck) || 
//                                  formState.errors.root.message?.includes(coverageOrErrorCheck) ));


//     if (hasVisibleFormErrors) { setQuoteResult({ ok: false, message: "Please correct form errors to see a quote.", warnings: [] }); return; }
...
//     if (totalTripDaysCalculated !== sumZoneDaysFromForm) { setQuoteResult({ ok: false, message: `Zone days sum (${sumZoneDaysFromForm}) does not match total trip days (${totalTripDaysCalculated}).`, warnings: [] }); return; }

//     const payload: GenerateQuotePayload = {
//       trip_start_date: startDate.toDate(), trip_end_date: endDate.toDate(),
//       trip_country: formValues.trip_countries[0] as string,
//       travellers_count: (formValues.travellers?.length > 0 ? formValues.travellers.length : 1) as Integer,
//       medical_coverage: formValues.emergency_medical_coverage as GenerateQuotePayload['medical_coverage'],
//       pa_coverage: formValues.personal_accident_coverage_level as GenerateQuotePayload['pa_coverage'],
...
//   }, [...watchedValuesForQuote, formState.errors, getValidationMessage]); // Added getValidationMessage to deps

//   const onSubmitForm = (data: InsuranceFormValues) => {
//     const finalTravellers = [...data.travellers];
//     if (finalTravellers.length > 0) {
//       finalTravellers[0] = { ...finalTravellers[0], first_name: getValues("c_first_name") || "", last_name: getValues("c_last_name") || "", birthdate: getValues("c_birthdate") || "", passport_number: getValues("c_passport_number") || "", passport_expiry_date: getValues("c_passport_expiry_date") || "", };
//     } else {
//       finalTravellers.push({ first_name: getValues("c_first_name") || "", last_name: getValues("c_last_name") || "", birthdate: getValues("c_birthdate") || "", passport_number: getValues("c_passport_number") || "", passport_expiry_date: getValues("c_passport_expiry_date") || "", });
//     }
//     // const finalData = {
//     //   ...data, travellers: finalTravellers.map(traveller => ({ first_name: traveller.first_name, last_name: traveller.last_name, birthdate: traveller.birthdate, passport_number: traveller.passport_number, passport_expiry_date: traveller.passport_expiry_date, })),
//     //   c_phone: `${getValues("c_phone_code") || ''}${getValues("c_phone_number") || ''}`,
//     //   c_whats_app: getValues("c_is_whatsapp_same_as_phone") ? `${getValues("c_phone_code") || ''}${getValues("c_phone_number") || ''}` : `${getValues("c_whats_app_code") || ''}${getValues("c_whats_app_number") || ''}`,
//     //   emergency_contact_phone: `${getValues("emergency_contact_phone_code") || ''}${getValues("emergency_contact_phone_number") || ''}`,
//     //   green_zone_days: Number(data.green_zone_days), amber_zone_days: Number(data.amber_zone_days), red_zone_days: Number(data.red_zone_days), black_zone_days: data.black_zone_days ? Number(data.black_zone_days) : 0,
//     //   medical_conditions: data.has_medical_conditions ? data.medical_conditions?.filter(item => item && item.trim() !== "") : [],
//     //   allergies: data.has_allergies ? data.allergies?.filter(item => item && item.trim() !== "") : [],
//     //   current_medications: data.has_current_medications ? data.current_medications?.filter(item => item && item.trim() !== "") : [],
//     //   trip_cities: data.trip_cities ? data.trip_cities.filter(city => city.name && city.name.trim() !== "") : [],
//     //   c_passport_number: getValues("c_passport_number"), c_passport_expiry_date: getValues("c_passport_expiry_date"), c_is_whatsapp_same_as_phone: getValues("c_is_whatsapp_same_as_phone"),
//     // };
//     alert(t("insuranceForm.submissionAlert")); 
//   };
...
//     let overallValidationPassed = true;
//     const partFieldsToValidate: Path<InsuranceFormValues>[] = [];
//     if (step === 1) partFieldsToValidate.push("c_phone_code", "c_phone_number", "c_whats_app_code", "c_whats_app_number", "c_passport_number", "c_passport_expiry_date");
...
//   const watchedPathsForSummary = ["trip_start_date", "trip_end_date", "emergency_medical_coverage", "personal_accident_coverage_level", "add_transit_coverage", "c_first_name", "c_last_name", "c_birthdate", "c_nationality", "trip_purpose", "primary_cities_regions_ukraine", "emergency_contact_first_name", "emergency_contact_last_name", "emergency_contact_phone", "c_passport_number", "c_passport_expiry_date"] as const;
//   const watchedValuesForSummary = watch(watchedPathsForSummary);
//   const watchedTravellersForSummary = watch("travellers");
//   const watchedTripCitiesForSummary = watch("trip_cities");

//   // Use 't' for UI labels, 'getValidationMessage' for option labels derived from validation messages/options section
...
//     if (quoteResult && !quoteResult.ok && quoteResult.message) {
//       if (quoteResult.message !== "Please correct form errors to see a quote.") { 
//           return (<div className="mt-1 text-xs text-red-600"><strong>{t("insuranceForm.quoteError")}:</strong> {quoteResult.message}</div>);
//       }
//     }
//     return null;
//   };
...
//                   <DatePickerField label={t("insuranceForm.travelEndDate")} name="trip_end_date" control={control} error={getError("trip_end_date")} placeholder={t("insuranceForm.selectEndDate")} minDate={watchedStartDate ? dayjs(watchedStartDate).startOf("day").toDate() : dayjs().startOf("day").toDate()}/>
//                 </div>
...
//                 <div className="space-y-4 mb-6">
//                   <SelectWithLabel label={t("insuranceForm.emergencyMedical")} name="emergency_medical_coverage" control={control} options={currentEmergencyMedicalCoverageOptions} placeholder={t("insuranceForm.selectMedicalCoverage")} error={getError("emergency_medical_coverage")} readOnly={coverageDisabled}/>
//                   <SelectWithLabel label={t("insuranceForm.personalAccident")} name="personal_accident_coverage_level" control={control} options={currentPersonalAccidentCoverageOptions} placeholder={t("insuranceForm.selectPACoverage")} error={getError("personal_accident_coverage_level")} readOnly={coverageDisabled}/>
//                   <TooltipProvider>
//                     <div className="flex items-center space-x-2">
//                       <Controller name="add_transit_coverage" control={control} render={({ field }) => (<Checkbox id="add_transit_coverage" checked={field.value} onCheckedChange={field.onChange} disabled={coverageDisabled}/>)}/>
//                       <Tooltip>
//                         <TooltipTrigger asChild><Label htmlFor="add_transit_coverage" className={coverageDisabled ? "text-gray-400" : ""}>{t("insuranceForm.addTransitCover")} <strong>€25</strong></Label></TooltipTrigger>
//                         <TooltipContent side="bottom" align="start" sideOffset={4}><div className="relative"><div className="bg-muted px-3 py-2 text-sm shadow">{t("insuranceForm.addTransitCoverTooltip")}</div><div className="absolute top-0 left-2.5 -translate-y-full w-0 h-0 border-l-8 border-r-8 border-b-8 border-l-transparent border-r-transparent border-b-muted"/></div></TooltipContent>
//                       </Tooltip>
//                     </div>
//                   </TooltipProvider>
//                 </div>
//                 <Button type="button" onClick={() => setShowQuote(true)} className="mt-6 w-full">{t("insuranceForm.actions.getQuote")}</Button> 
//                 {showQuote && (
//                   <div className="mt-8 p-6 bg-gray-50 rounded-md border">
...
//                     {renderQuoteWarnings()}
//                     <div className="text-sm mt-2">
//                       <p>{t("insuranceForm.medical")}: {getEmergencyMedicalLabel(watch("emergency_medical_coverage"))}</p>
//                       <p>{t("insuranceForm.pa")}: {getPALabel(watch("personal_accident_coverage_level"))}</p>
//                       <p>{t("insuranceForm.transit")}: {watch("add_transit_coverage") ? formatEuro(pricing.transitCost) : t("insuranceForm.no")}</p>
//                     </div>
//                   </div>
//                 )}
//               </>
...
//                   <div><p className="text-sm font-medium text-gray-900 mb-1">{t('insuranceForm.step1.phoneNumber')}</p><div className="flex items-start gap-2"><div className="w-1/3 shrink-0"><SelectWithLabel control={control} name="c_phone_code" label="" options={currentCountryCodeOptions} placeholder={t("insuranceForm.step3.codePlaceholder")} error={getError("c_phone_code")}/></div><div className="flex-grow"><InputWithLabel label="" name="c_phone_number" type="tel" register={register} placeholder={t('insuranceForm.step1.enterNumber')} error={getError("c_phone_number")}/></div></div>{getError("c_phone") && <p className="text-sm text-red-500 mt-1">{getError("c_phone")?.message}</p>}</div>
//                   <div><p className="text-sm font-medium text-gray-900 mb-1">{t('insuranceForm.step1.whatsapp')}</p><div className="flex items-start gap-2"><div className="w-1/3 shrink-0"><SelectWithLabel control={control} name="c_whats_app_code" label="" options={currentCountryCodeOptions} placeholder={t("insuranceForm.step3.codePlaceholder")} readOnly={cIsWhatsAppSameAsPhone} error={getError("c_whats_app_code")}/></div><div className="flex-grow"><InputWithLabel label="" name="c_whats_app_number" type="tel" register={register} placeholder={t('insuranceForm.step1.enterNumber')} readOnly={cIsWhatsAppSameAsPhone} error={getError("c_whats_app_number")}/></div></div>{getError("c_whats_app") && <p className="text-sm text-red-500 mt-1">{getError("c_whats_app")?.message}</p>}</div>
//                   <div className="md:col-span-2 mt-2"><div className="flex items-center space-x-2"><Controller name="c_is_whatsapp_same_as_phone" control={control} render={({ field }) => (<Checkbox id="c_is_whatsapp_same_as_phone" checked={field.value} onCheckedChange={field.onChange}/>)}/><Label htmlFor="c_is_whatsapp_same_as_phone">{t("insuranceForm.step1.whatsappSameAsPhone")}</Label></div></div>
//                   <InputWithLabel label={t('insuranceForm.step1.email')} name="c_email" type="email" register={register} error={getError("c_email")}/>
...
//                   <SelectWithLabel label={t('insuranceForm.step1.countryOfResidence')} name="city_of_residence" control={control} options={currentCountryOptions} placeholder={t('insuranceForm.step1.selectCountry')} error={getError("city_of_residence")}/>
//                 </div>
//                 {travellerFields.map((field, index) => {
//                   if (index === 0) return null;
//                   const firstNamePath = `travellers.${index}.first_name` as Path<InsuranceFormValues>;
//                   const lastNamePath = `travellers.${index}.last_name` as Path<InsuranceFormValues>;
//                   const birthdatePath = `travellers.${index}.birthdate` as Path<InsuranceFormValues>;
//                   return (<div key={field.id} className="mt-6 pt-6 border-t"><div className="flex justify-between items-center mb-3"><h3 className="text-lg font-semibold text-[#1A2C50]">{t(`insuranceForm.step1.additionalTraveller`)}</h3><Button type="button" variant="destructive" size="sm" onClick={() => removeTraveller(index)}>{t("insuranceForm.step1.remove")}</Button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><div className="flex items-start gap-2"><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactFirstName")} name={firstNamePath} register={register} placeholder={t("insuranceForm.step1.contactFirstName")} error={getError(firstNamePath)}/></div><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactLastName")} name={lastNamePath} register={register} placeholder={t("insuranceForm.step1.contactLastName")} error={getError(lastNamePath)}/></div></div></div><div className="w-full mt-1.5"><BirthDateField label={t('insuranceForm.step1.dob')} name={birthdatePath} control={control} getError={getError} watch={watch}/></div></div></div>);
//                 })}
//                 <Button type="button" variant="outline" onClick={() => appendTraveller({ first_name: "", last_name: "", birthdate: "", passport_number: "", passport_expiry_date: "" })} className="mt-6">{t("insuranceForm.step1.addAdditionalTraveller")}</Button>
...
//               {step === steps.length - 1 && (<Button type="submit" className="w-full sm:w-auto px-8 py-3 text-base bg-green-600 hover:bg-green-700 text-white" disabled={formState.isSubmitting}>{formState.isSubmitting ? t("insuranceForm.actions.processing") : t("insuranceForm.actions.confirm")}</Button>)}
//             </div>
//           </form>
//         </div>
//       </div>
...
//   SelectWithLabel,
//   DatePickerField
// } from "./FormFields"; // Assuming FormFields is in the same directory or adjust path
// import { format as formatDateFn } from "date-fns";
// import BirthDateField from "../form/BirthDateField"; // Assuming BirthDateField is in ../form/ or adjust path
// import { useTranslation } from "@/hooks/useTranslation";
// import { useValidationMessages } from "@/lib/useValidationMessages";
// import dayjs from 'dayjs';
// import { Tooltip, TooltipContent, TooltipProvider } from "../ui/tooltip"; // Assuming Tooltip is in ../ui/ or adjust path
// import { TooltipTrigger } from "@radix-ui/react-tooltip";
...
//   let medicalCoverAmount = 0; let paCoverAmount = 0; const warnings: string[] = [];
//   if (medicalRates && medicalCoverageLevel !== "0") {
//     medicalCoverAmount += (medicalRates["greenME"] || 0) * data.green_zone_days; medicalCoverAmount += (medicalRates["amberME"] || 0) * data.amber_zone_days; medicalCoverAmount += (medicalRates["redME"] || 0) * data.red_zone_days; medicalCoverAmount += (medicalRates["blackME"] || 0) * data.black_zone_days;
//     if (data.green_zone_days > 0 && (!medicalRates["greenME"] || medicalRates["greenME"] <= 0)) { warnings.push("Medical coverage for green zone is not available or has zero cost for this coverage amount.");}
//     if (data.amber_zone_days > 0 && (!medicalRates["amberME"] || medicalRates["amberME"] <= 0)) { warnings.push("Medical coverage for amber zone is not available or has zero cost for this coverage amount.");}
//     if (data.red_zone_days > 0 && (!medicalRates["redME"] || medicalRates["redME"] <= 0)) { warnings.push("Medical coverage for red zone is not available or has zero cost for this coverage amount.");}
//     if (data.black_zone_days > 0 && (!medicalRates["blackME"] || medicalRates["blackME"] <= 0)) { warnings.push("Medical coverage for black zone is not available or has zero cost for this coverage amount.");}
//   }
//   if (paRates && paCoverageLevel !== "0") {
//     paCoverAmount += (paRates["greenPA"] || 0) * data.green_zone_days; paCoverAmount += (paRates["amberPA"] || 0) * data.amber_zone_days; paCoverAmount += (paRates["redPA"] || 0) * data.red_zone_days; paCoverAmount += (paRates["blackPA"] || 0) * data.black_zone_days;
//     if (data.green_zone_days > 0 && (!paRates["greenPA"] || paRates["greenPA"] <= 0)) { warnings.push("Personal Accident coverage for green zone is not available or has zero cost for this coverage amount.");}
//     if (data.amber_zone_days > 0 && (!paRates["amberPA"] || paRates["amberPA"] <= 0)) { warnings.push("Personal Accident coverage for amber zone is not available or has zero cost for this coverage amount.");}
//     if (data.red_zone_days > 0 && (!paRates["redPA"] || paRates["redPA"] <= 0)) { warnings.push("Personal Accident coverage for red zone is not available or has zero cost for this coverage amount.");}
//     if (data.black_zone_days > 0 && (!paRates["blackPA"] || paRates["blackPA"] <= 0)) { warnings.push("Personal Accident coverage for black zone is not available or has zero cost for this coverage amount.");}
//   }
//   let totalAmount = medicalCoverAmount + paCoverAmount;
//   if (data.travellers_count > 0) totalAmount *= data.travellers_count;
//   if (data.transit_coverage) totalAmount += pricing.transitCost;
...
//   const { fields: travellerFields, append: appendTraveller, remove: removeTraveller } = useFieldArray<InsuranceFormValues, "travellers", "id">({ control, name: "travellers" });
//   const { fields: cityFields, append: appendCity, remove: removeCity } = useFieldArray<InsuranceFormValues, "trip_cities", "id">({ control, name: "trip_cities" });
...
//     const currentTravellers = getValues("travellers");
//     if (!currentTravellers || currentTravellers.length === 0) {
//       setValue("travellers", [{ first_name: cFirstNameValue || "", last_name: cLastNameValue || "", birthdate: cBirthdateValue || "", passport_number: cPassportNumberValue || "", passport_expiry_date: cPassportExpiryDateValue || "" }], { shouldValidate: false, shouldDirty: false });
//     } else {
//       let changed = false; const newTraveller0 = { ...currentTravellers[0] };
//       if (newTraveller0.first_name !== cFirstNameValue) { newTraveller0.first_name = cFirstNameValue || ""; changed = true; }
//       if (newTraveller0.last_name !== cLastNameValue) { newTraveller0.last_name = cLastNameValue || ""; changed = true; }
//       if (newTraveller0.birthdate !== cBirthdateValue) { newTraveller0.birthdate = cBirthdateValue || ""; changed = true; }
//       if (newTraveller0.passport_number !== cPassportNumberValue) { newTraveller0.passport_number = cPassportNumberValue || ""; changed = true; }
//       if (newTraveller0.passport_expiry_date !== cPassportExpiryDateValue) { newTraveller0.passport_expiry_date = cPassportExpiryDateValue || ""; changed = true; }
//       if (changed) { const updatedTravellers = [...currentTravellers]; updatedTravellers[0] = newTraveller0; setValue("travellers", updatedTravellers, { shouldValidate: step === 1 && (!!formState.dirtyFields.c_first_name || !!formState.dirtyFields.c_last_name || !!formState.dirtyFields.c_birthdate || !!formState.dirtyFields.c_passport_number || !!formState.dirtyFields.c_passport_expiry_date), shouldDirty: true }); }
//     }
//   }, [cFirstNameValue, cLastNameValue, cBirthdateValue, cPassportNumberValue, cPassportExpiryDateValue, setValue, getValues, step, formState.dirtyFields]);

//   useEffect(() => {
//     if (watchedStartDate && watchedEndDate) {
//       const start = dayjs(watchedStartDate + "T00:00:00"); const end = dayjs(watchedEndDate + "T00:00:00");
//       if (start.isValid() && end.isValid() && (end.isSame(start) || end.isAfter(start))) {
//         const diffDays = end.diff(start, 'day') + 1; setCalculatedTotalRiskZoneDays(diffDays); setValue("green_zone_days", diffDays, { shouldValidate: true, shouldDirty: true });
//         if (Number(getValues("amber_zone_days")) !== 0) setValue("amber_zone_days", 0, { shouldValidate: true, shouldDirty: true });
//         if (Number(getValues("red_zone_days")) !== 0) setValue("red_zone_days", 0, { shouldValidate: true, shouldDirty: true });
//       } else { setCalculatedTotalRiskZoneDays(null); if (Number(getValues("green_zone_days")) !== 0) setValue("green_zone_days", 0, { shouldValidate: true, shouldDirty: true }); }
//     } else { setCalculatedTotalRiskZoneDays(null); if (Number(getValues("green_zone_days")) !== 0) setValue("green_zone_days", 0, { shouldValidate: true, shouldDirty: true }); }
//   }, [watchedStartDate, watchedEndDate, setValue, getValues]);
...
//     const subscription = watch((currentValues, { name, type }) => {
//       if ((name === "amber_zone_days" || name === "red_zone_days") && type === "change" && calculatedTotalRiskZoneDays !== null) {
//         let amber = Number(currentValues.amber_zone_days || 0); let red = Number(currentValues.red_zone_days || 0);
//         amber = Math.max(0, Math.floor(amber)); red = Math.max(0, Math.floor(red));
//         if (amber + red > calculatedTotalRiskZoneDays) {
//           if (name === "amber_zone_days") { amber = calculatedTotalRiskZoneDays - red; setValue("amber_zone_days", Math.max(0, amber), { shouldValidate: true, shouldDirty: true }); }
//           else if (name === "red_zone_days") { red = calculatedTotalRiskZoneDays - amber; setValue("red_zone_days", Math.max(0, red), { shouldValidate: true, shouldDirty: true }); }
//         }
//         const newGreenDays = calculatedTotalRiskZoneDays - amber - red;
//         if (Number(getValues("green_zone_days")) !== Math.max(0, newGreenDays)) { setValue("green_zone_days", Math.max(0, newGreenDays), { shouldValidate: true, shouldDirty: true }); }
//       }
//     });
//     return () => subscription.unsubscribe();
//   }, [watch, setValue, getValues, calculatedTotalRiskZoneDays]);

//   const cPhoneCode = watch("c_phone_code"); const cPhoneNumber = watch("c_phone_number"); const cIsWhatsAppSameAsPhone = watch("c_is_whatsapp_same_as_phone");
//   useEffect(() => { const fullNumber = `${cPhoneCode || ''}${cPhoneNumber || ''}`; if (getValues("c_phone") !== fullNumber) setValue("c_phone", fullNumber, { shouldValidate: true, shouldDirty: true }); }, [cPhoneCode, cPhoneNumber, setValue, getValues]);
//   useEffect(() => { if (cIsWhatsAppSameAsPhone) { setValue("c_whats_app_code", cPhoneCode || "", { shouldValidate: true, shouldDirty: true }); setValue("c_whats_app_number", cPhoneNumber || "", { shouldValidate: true, shouldDirty: true }); } }, [cIsWhatsAppSameAsPhone, cPhoneCode, cPhoneNumber, setValue]);
//   const cWhatsAppCode = watch("c_whats_app_code"); const cWhatsAppNumber = watch("c_whats_app_number");
//   useEffect(() => { const fullNumber = `${cWhatsAppCode || ''}${cWhatsAppNumber || ''}`; if (getValues("c_whats_app") !== fullNumber) setValue("c_whats_app", fullNumber, { shouldValidate: true, shouldDirty: true }); }, [cWhatsAppCode, cWhatsAppNumber, setValue, getValues]);
//   const emergencyContactPhoneCode = watch("emergency_contact_phone_code"); const emergencyContactPhoneNumber = watch("emergency_contact_phone_number");
//   useEffect(() => { const fullNumber = `${emergencyContactPhoneCode || ''}${emergencyContactPhoneNumber || ''}`; if (getValues("emergency_contact_phone") !== fullNumber) setValue("emergency_contact_phone", fullNumber, { shouldValidate: true, shouldDirty: true }); }, [emergencyContactPhoneCode, emergencyContactPhoneNumber, setValue, getValues]);
...
//     const hasRequiredFieldsForQuote = formValues.trip_start_date && formValues.trip_end_date && formValues.trip_countries && formValues.trip_countries.length > 0 && formValues.trip_countries[0] && formValues.emergency_medical_coverage && formValues.personal_accident_coverage_level !== undefined && formValues.travellers && formValues.travellers.length > 0;
//     if (!hasRequiredFieldsForQuote) { setQuoteResult(null); return; }
//     const startDate = dayjs(formValues.trip_start_date + "T00:00:00"); const endDate = dayjs(formValues.trip_end_date + "T00:00:00");
//     if (!startDate.isValid() || !endDate.isValid() || endDate.isBefore(startDate)) { setQuoteResult({ ok: false, message: "Invalid trip dates for quote calculation.", warnings: [] }); return; }
//     const zoneDaysSumErrorCheck = getValidationMessage('insuranceForm.validationzoneDaysSum', {sumOfZoneDays:0, totalCalculatedDays:0}).substring(0,20);
//     const coverageOrErrorCheck = getValidationMessage('insuranceForm.validationcoverageOr').substring(0,20);
//     const criticalErrorFields: (Path<InsuranceFormValues>)[] = ["trip_start_date", "trip_end_date", "green_zone_days", "amber_zone_days", "red_zone_days", "trip_countries", "emergency_medical_coverage", "personal_accident_coverage_level"];
//     const hasVisibleFormErrors = criticalErrorFields.some(field => get(formState.errors, field)) || (formState.errors.root && (formState.errors.root.message?.includes(zoneDaysSumErrorCheck) || formState.errors.root.message?.includes(coverageOrErrorCheck) ));
//     if (hasVisibleFormErrors) { setQuoteResult({ ok: false, message: "Please correct form errors to see a quote.", warnings: [] }); return; }
//     const totalTripDaysCalculated = endDate.diff(startDate, 'day') + 1;
//     const sumZoneDaysFromForm = Number(formValues.green_zone_days || 0) + Number(formValues.amber_zone_days || 0) + Number(formValues.red_zone_days || 0) + Number(formValues.black_zone_days || 0);
//     if (totalTripDaysCalculated !== sumZoneDaysFromForm) { setQuoteResult({ ok: false, message: `Zone days sum (${sumZoneDaysFromForm}) does not match total trip days (${totalTripDaysCalculated}).`, warnings: [] }); return; }
//     const payload: GenerateQuotePayload = { trip_start_date: startDate.toDate(), trip_end_date: endDate.toDate(), trip_country: formValues.trip_countries[0] as string, travellers_count: (formValues.travellers?.length > 0 ? formValues.travellers.length : 1) as Integer, medical_coverage: formValues.emergency_medical_coverage as GenerateQuotePayload['medical_coverage'], pa_coverage: formValues.personal_accident_coverage_level as GenerateQuotePayload['pa_coverage'], transit_coverage: formValues.add_transit_coverage, green_zone_days: Number(formValues.green_zone_days || 0) as Integer, amber_zone_days: Number(formValues.amber_zone_days || 0) as Integer, red_zone_days: Number(formValues.red_zone_days || 0) as Integer, black_zone_days: Number(formValues.black_zone_days || 0) as Integer };
//     const result = generateQuote(payload); setQuoteResult(result);
//   // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [...watchedValuesForQuote, formState.errors, getValidationMessage]);


//   const onSubmitForm = (data: InsuranceFormValues) => {
//     // Your original finalTravellers logic - keep if needed for other purposes (e.g. backend submission)
//     const finalTravellers = [...data.travellers];
//     if (finalTravellers.length > 0) {
//       finalTravellers[0] = { ...finalTravellers[0], first_name: getValues("c_first_name") || "", last_name: getValues("c_last_name") || "", birthdate: getValues("c_birthdate") || "", passport_number: getValues("c_passport_number") || "", passport_expiry_date: getValues("c_passport_expiry_date") || "" };
//     } else {
...
//     const formData = getValues(); // Get current form values for payer details

//     // Crucial: Ensure these values are not empty or invalid.
//     // Your Joi schema should enforce this.
//     const payerFirstName = formData.c_first_name;
//     const payerLastName = formData.c_last_name;
//     const payerEmail = formData.c_email;

//     console.log("[InsuranceForm] Payer Info before sending to Flywire URL:", { payerFirstName, payerLastName, payerEmail });

//     // Validate essential payer info before proceeding
//     if (!payerFirstName || !payerLastName || !payerEmail) {
//         alert(t("insuranceForm.errors.missingPayerInfoForPayment") || "Payer first name, last name, and email are required to proceed with payment.");
//         return;
//     }
//     // Basic email format check (more robust validation should be in Joi schema)
//     if (!/^\S+@\S+\.\S+$/.test(payerEmail)) {
//         alert(t("insuranceForm.errors.invalidEmailForPayment") || "Please provide a valid email address for payment.");
//         return;
//     }


//     if (!quoteResult || !quoteResult.ok || !quoteResult.data) {
//       alert(t("insuranceForm.errors.quoteNotAvailableForPayment") || "Quote is not available or contains errors. Cannot proceed to payment.");
//       return;
//     }
//     const paymentAmount = quoteResult.data.totalAmount;
//     const generatedInvoiceNumber = `INV-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;

//     if (paymentAmount <= 0) {
//         alert(t("insuranceForm.errors.invalidAmountForPayment") || "Payment amount must be greater than zero. Cannot proceed.");
//         return;
//     }

//     if (typeof window === "undefined") {
//         alert("Cannot initiate payment: Window object not found."); // Should not happen in client component
//         return;
//     }

//     const flywireCheckoutUrl = new URL('/flywire-checkout', window.location.origin);
//     flywireCheckoutUrl.searchParams.append('firstName', payerFirstName); // Pass directly, validation happened above
//     flywireCheckoutUrl.searchParams.append('lastName', payerLastName);   // Pass directly
//     flywireCheckoutUrl.searchParams.append('email', payerEmail);         // Pass directly
//     flywireCheckoutUrl.searchParams.append('amount', paymentAmount.toString());
//     flywireCheckoutUrl.searchParams.append('invoiceNumber', generatedInvoiceNumber);

//     console.log("[InsuranceForm] Opening Flywire URL:", flywireCheckoutUrl.toString());

//     window.open(flywireCheckoutUrl.toString(), '_blank');
//     alert(t("insuranceForm.paymentInitiatedAlert") || "Payment process initiated in a new tab. Please complete your payment there.");

//     // Optional: Submit form data to your backend here if needed
//     // console.log("Submitting data to backend (example):", { ...data, finalTravellers, quote: quoteResult.data, invoiceNumber: generatedInvoiceNumber });
//     // await fetch('/api/submit-insurance', { method: 'POST', body: JSON.stringify(...) });
//   };
...
//     if (step === 1) partFieldsToValidate.push("c_phone_code", "c_phone_number", "c_whats_app_code", "c_whats_app_number", "c_passport_number", "c_passport_expiry_date");
//     if (step === 3) partFieldsToValidate.push("emergency_contact_phone_code", "emergency_contact_phone_number");
//     if (partFieldsToValidate.length > 0) { const partValidationResult = await trigger(partFieldsToValidate, { shouldFocus: true }); if (!partValidationResult) overallValidationPassed = false; }
//     if (overallValidationPassed) { const currentStepValidationResult = await trigger(currentStepFields, { shouldFocus: true }); if (!currentStepValidationResult) overallValidationPassed = false; }
//     if (overallValidationPassed && step === 0) {
//       const values = getValues();
//       if (values.trip_start_date && values.trip_end_date) {
//         const startDate = dayjs(values.trip_start_date + "T00:00:00"); const endDate = dayjs(values.trip_end_date + "T00:00:00");
//         if (startDate.isValid() && endDate.isValid() && (endDate.isSame(startDate) || endDate.isAfter(startDate))) {
//           const totalTripDays = endDate.diff(startDate, 'day') + 1;
//           const sumZoneDays = Number(values.green_zone_days || 0) + Number(values.amber_zone_days || 0) + Number(values.red_zone_days || 0) + Number(values.black_zone_days || 0);
//           if (totalTripDays !== sumZoneDays) { const message = getValidationMessage('insuranceForm.validationzoneDaysSum', { totalCalculatedDays: totalTripDays, sumOfZoneDays: sumZoneDays }); form.setError("root", { type: "manual", message }); overallValidationPassed = false; }
...
//       const fieldsToSearchForErrors = Array.from(new Set([...currentStepFields, ...partFieldsToValidate]));
//       let firstErrorKeyFound: Path<InsuranceFormValues> | undefined = fieldsToSearchForErrors.find(fieldName => getError(fieldName) !== undefined);
//       if (!firstErrorKeyFound && step === 1 && formState.errors.travellers) { /* ... error finding logic ... */ }
//       if (!firstErrorKeyFound && step === 2 && formState.errors.trip_cities) { /* ... error finding logic ... */ }
//       if (!firstErrorKeyFound && formState.errors.root?.message) { const zoneDayElement = document.querySelector(`[name='amber_zone_days']`) || document.querySelector(`[name='red_zone_days']`) || document.querySelector(`[name='green_zone_days']`); zoneDayElement?.scrollIntoView({ behavior: 'smooth', block: 'center' }); return; }
//       if (firstErrorKeyFound) { const element = document.querySelector(`[name='${firstErrorKeyFound}']`) || document.getElementById(firstErrorKeyFound as string); element?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }
//       return;
//     }
//     if (step < steps.length - 1) setStep((prev) => prev + 1);
//   };
...
//   const handlePrevStep = () => setStep((prev) => Math.max(prev - 1, 0));
//   const getError = (fieldName: Path<InsuranceFormValues>): FieldError | undefined => { const error = get(formState.errors, fieldName); if (error && typeof error === 'object' && 'message' in error && typeof error.message === 'string') return error as FieldError; return undefined; };
//   const calculateAge = (birthdate: string | undefined): string => { if (!birthdate) return ""; const parts = birthdate.split('-'); if (parts.length !== 3) return ""; const year = parseInt(parts[0], 10), month = parseInt(parts[1], 10) - 1, day = parseInt(parts[2], 10); if (isNaN(year) || isNaN(month) || isNaN(day)) return ""; const birthDateObj = new Date(year, month, day); if (isNaN(birthDateObj.getTime()) || birthDateObj.getFullYear() !== year || birthDateObj.getMonth() !== month || birthDateObj.getDate() !== day) return ""; const today = new Date(); let age = today.getFullYear() - birthDateObj.getFullYear(); const m = today.getMonth() - birthDateObj.getMonth(); if (m < 0 || (m === 0 && today.getDate() < birthDateObj.getDate())) age--; return age >= 0 ? age.toString() : ""; };
//   const formatDateForDisplay = (dateString: string | undefined): string => { if (!dateString) return "N/A"; try { const parts = dateString.split('-'); if (parts.length !== 3) return "Invalid Date"; const year = parseInt(parts[0], 10), month = parseInt(parts[1], 10) - 1, day = parseInt(parts[2], 10); const dateObj = new Date(year, month, day); if (isNaN(dateObj.getTime()) || dateObj.getFullYear() !== year || dateObj.getMonth() !== month || dateObj.getDate() !== day) return "Invalid Date"; return formatDateFn(dateObj, "PPP"); } catch (e) { console.error("Error formatting date:", dateString, e); return "Invalid Date"; } };
//   const watchedPathsForSummary = ["trip_start_date", "trip_end_date", "emergency_medical_coverage", "personal_accident_coverage_level", "add_transit_coverage", "c_first_name", "c_last_name", "c_birthdate", "c_nationality", "trip_purpose", "primary_cities_regions_ukraine", "emergency_contact_first_name", "emergency_contact_last_name", "emergency_contact_phone", "c_passport_number", "c_passport_expiry_date"] as const;
//   const watchedValuesForSummary = watch(watchedPathsForSummary);
//   const watchedTravellersForSummary = watch("travellers");
//   const watchedTripCitiesForSummary = watch("trip_cities");
...
//   const getTripPurposeLabel = (value: string) => currentTripPurposes.find(opt => opt.value === value)?.label || value || "N/A";
//   const getNationalityLabel = (value: string) => currentNationalityOptions.find(opt => opt.value === value)?.label || value || "N/A";
//   const formatFullName = (firstName?: string, lastName?: string): string => { const first = firstName || "", last = lastName || ""; if (first && last) return `${first} ${last}`; return first || last || "N/A"; };
//   const renderQuoteDisplay = () => { if (!quoteResult) return "€0.00 "; if (!quoteResult.ok) return `€0.00 (${quoteResult.message || "Error"})`; if (quoteResult.data) return `€${quoteResult.data.totalAmount.toFixed(2)}`; return "€0.00 (Unavailable)"; };
//   const renderQuoteWarnings = () => { if (quoteResult && quoteResult.ok && quoteResult.warnings && quoteResult.warnings.length > 0) { return (<div className="mt-1 text-xs text-orange-600"><strong>{t("insuranceForm.warnings")}:</strong><ul className="list-disc list-inside pl-4">{quoteResult.warnings.map((warning, idx) => <li key={idx}>{warning}</li>)}</ul></div>); } if (quoteResult && !quoteResult.ok && quoteResult.message) { if (quoteResult.message !== "Please correct form errors to see a quote.") { return (<div className="mt-1 text-xs text-red-600"><strong>{t("insuranceForm.quoteError")}:</strong> {quoteResult.message}</div>); } } return null; };



//   return (
//     <div className="flex justify-center px-4 py-10 "  style={{ backgroundImage: "url('/bg.jpg')", backgroundSize: "cover", backgroundPosition: "center" }}>
//       <div className="w-full max-w-4xl">
//         <div className="mb-8 space-y-4">
//           <div className="flex items-center justify-between">
//             {steps.map((label, index) => ( <div key={index} className="flex-1 text-center text-sm font-semibold"> <div className={`w-8 h-8 mx-auto rounded-full flex items-center justify-center ${index <= step ? "bg-[#1A2C50] text-white" : "bg-[#00BBD3] text-white"}`}>{index + 1}</div> <div className="mt-1 font-semibold">{label}</div> </div> ))}
//           </div>
//           <div className="h-2 bg-[#00BBD3] rounded-full"><div className="h-2 bg-[#1A2C50] rounded-full transition-all duration-300" style={{ width: `${((step + 1) / steps.length) * 100}%` }}/></div>
//         </div>
//         <div className="bg-white/90 p-6 md:p-8 shadow-lg shadow-[#1A2C50] rounded-md">
//           <form onSubmit={form.handleSubmit(onSubmitForm)} className="space-y-8">
//             {/* STEP 0 UI */}
//             {step === 0 && ( <> <h2 className="text-2xl font-semibold mb-6 text-[#1A2C50]">{t("insuranceForm.travelDetails")}</h2> <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"> <DatePickerField label={t("insuranceForm.travelStartDate")} name="trip_start_date" control={control} error={getError("trip_start_date")} placeholder={t("insuranceForm.selectStartDate")} minDate={dayjs().startOf('day').toDate()} maxDate={watchedEndDate ? dayjs(watchedEndDate).toDate() : undefined}/> <DatePickerField label={t("insuranceForm.travelEndDate")} name="trip_end_date" control={control} error={getError("trip_end_date")} placeholder={t("insuranceForm.selectEndDate")} minDate={watchedStartDate ? dayjs(watchedStartDate).startOf("day").toDate() : dayjs().startOf("day").toDate()}/> </div> <h3 className="text-xl font-semibold mb-1 text-[#1A2C50]">{t("insuranceForm.riskZoneDays")}</h3> <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"> <InputWithLabel label={t("insuranceForm.totalDays")} name="displayTotalDays" type="number" value={calculatedTotalRiskZoneDays !== null ? calculatedTotalRiskZoneDays.toString() : ""} readOnly={true}/> <InputWithLabel label={t("insuranceForm.greenZoneDays")} name="green_zone_days" type="number" value={watchedGreenDaysForDisplay?.toString()} readOnly={true} register={register} error={getError("green_zone_days")}/> <InputWithLabel label={t("insuranceForm.amberZoneDays")} name="amber_zone_days" type="number" register={register} error={getError("amber_zone_days")}/> <InputWithLabel label={t("insuranceForm.redZoneDays")} name="red_zone_days" type="number" register={register} error={getError("red_zone_days")}/> </div> {formState.errors.root && (<p className="text-sm text-red-500 mb-4">{formState.errors.root.message}</p>)} <SelectWithLabel label={t('insuranceForm.step1.countryTravellingTo')} name="trip_countries.0" control={control} options={currentCountryTravellingToOptions} placeholder={t('insuranceForm.step1.selectCountry')} error={getError("trip_countries.0" as Path<InsuranceFormValues>) || getError("trip_countries" as Path<InsuranceFormValues>)}/> <h3 className="text-xl font-semibold mb-4 text-[#1A2C50]">{t("insuranceForm.coverageOptions")}</h3> <div className="space-y-4 mb-6"> <SelectWithLabel label={t("insuranceForm.emergencyMedical")} name="emergency_medical_coverage" control={control} options={currentEmergencyMedicalCoverageOptions} placeholder={t("insuranceForm.selectMedicalCoverage")} error={getError("emergency_medical_coverage")} readOnly={coverageDisabled}/> <SelectWithLabel label={t("insuranceForm.personalAccident")} name="personal_accident_coverage_level" control={control} options={currentPersonalAccidentCoverageOptions} placeholder={t("insuranceForm.selectPACoverage")} error={getError("personal_accident_coverage_level")} readOnly={coverageDisabled}/> <TooltipProvider> <div className="flex items-center space-x-2"> <Controller name="add_transit_coverage" control={control} render={({ field }) => (<Checkbox id="add_transit_coverage" checked={field.value} onCheckedChange={field.onChange} disabled={coverageDisabled}/>)}/> <Tooltip> <TooltipTrigger asChild><Label htmlFor="add_transit_coverage" className={coverageDisabled ? "text-gray-400" : ""}>{t("insuranceForm.addTransitCover")} <strong>€25</strong></Label></TooltipTrigger> <TooltipContent side="bottom" align="start" sideOffset={4}><div className="relative"><div className="bg-muted px-3 py-2 text-sm shadow">{t("insuranceForm.addTransitCoverTooltip")}</div><div className="absolute top-0 left-2.5 -translate-y-full w-0 h-0 border-l-8 border-r-8 border-b-8 border-l-transparent border-r-transparent border-b-muted"/></div></TooltipContent> </Tooltip> </div> </TooltipProvider> </div> <Button type="button" onClick={() => setShowQuote(true)} className="mt-6 w-full">{t("insuranceForm.actions.getQuote")}</Button> {showQuote && ( <div className="mt-8 p-6 bg-gray-50 rounded-md border"> <h3 className="text-xl font-semibold text-[#1A2C50] mb-2">{t("insuranceForm.quoteSummary")}</h3> <p className="text-2xl font-bold text-[#00BBD3]">{renderQuoteDisplay()}</p> {renderQuoteWarnings()} <div className="text-sm mt-2"> <p>{t("insuranceForm.medical")}: {getEmergencyMedicalLabel(watch("emergency_medical_coverage"))}</p> <p>{t("insuranceForm.pa")}: {getPALabel(watch("personal_accident_coverage_level"))}</p> <p>{t("insuranceForm.transit")}: {watch("add_transit_coverage") ? formatEuro(pricing.transitCost) : t("insuranceForm.no")}</p> </div> </div> )} </>
//             )}
//             {/* STEP 1 UI */}
//             {step === 1 && ( <> <h2 className="text-2xl font-semibold mb-6 text-[#1A2C50]">{t('insuranceForm.step1.yourDetails')}</h2> <h3 className="text-lg font-semibold mb-3 text-[#1A2C50]">{t('insuranceForm.step1.primaryTraveller')}</h3> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div><div className="flex items-start gap-2"><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactFirstName")} name="c_first_name" register={register} placeholder={t("insuranceForm.step1.contactFirstName")} error={getError("c_first_name") || getError("travellers.0.first_name" as Path<InsuranceFormValues>)}/></div><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactLastName")} name="c_last_name" register={register} placeholder={t("insuranceForm.step1.contactLastName")} error={getError("c_last_name") || getError("travellers.0.last_name" as Path<InsuranceFormValues>)}/></div></div></div> <div className="mt-1.5"><BirthDateField label={t('insuranceForm.step1.dob')} name="c_birthdate" control={control} getError={getError} watch={watch}/></div> <div><p className="text-sm font-medium text-gray-900 mb-1">{t('insuranceForm.step1.phoneNumber')}</p><div className="flex items-start gap-2"><div className="w-1/3 shrink-0"><SelectWithLabel control={control} name="c_phone_code" label="" options={currentCountryCodeOptions} placeholder={t("insuranceForm.step3.codePlaceholder")} error={getError("c_phone_code")}/></div><div className="flex-grow"><InputWithLabel label="" name="c_phone_number" type="tel" register={register} placeholder={t('insuranceForm.step1.enterNumber')} error={getError("c_phone_number")}/></div></div>{getError("c_phone") && <p className="text-sm text-red-500 mt-1">{getError("c_phone")?.message}</p>}</div> <div><p className="text-sm font-medium text-gray-900 mb-1">{t('insuranceForm.step1.whatsapp')}</p><div className="flex items-start gap-2"><div className="w-1/3 shrink-0"><SelectWithLabel control={control} name="c_whats_app_code" label="" options={currentCountryCodeOptions} placeholder={t("insuranceForm.step3.codePlaceholder")} readOnly={cIsWhatsAppSameAsPhone} error={getError("c_whats_app_code")}/></div><div className="flex-grow"><InputWithLabel label="" name="c_whats_app_number" type="tel" register={register} placeholder={t('insuranceForm.step1.enterNumber')} readOnly={cIsWhatsAppSameAsPhone} error={getError("c_whats_app_number")}/></div></div>{getError("c_whats_app") && <p className="text-sm text-red-500 mt-1">{getError("c_whats_app")?.message}</p>}</div> <div className="md:col-span-2 mt-2"><div className="flex items-center space-x-2"><Controller name="c_is_whatsapp_same_as_phone" control={control} render={({ field }) => (<Checkbox id="c_is_whatsapp_same_as_phone" checked={field.value} onCheckedChange={field.onChange}/>)}/><Label htmlFor="c_is_whatsapp_same_as_phone">{t("insuranceForm.step1.whatsappSameAsPhone")}</Label></div></div> <InputWithLabel label={t('insuranceForm.step1.email')} name="c_email" type="email" register={register} error={getError("c_email")}/> <SelectWithLabel label={t('insuranceForm.step1.nationality')} name="c_nationality" control={control} options={currentNationalityOptions} placeholder={t('insuranceForm.step1.selectNationality')} error={getError("c_nationality")}/> <SelectWithLabel label={t('insuranceForm.step1.countryOfResidence')} name="city_of_residence" control={control} options={currentCountryOptions} placeholder={t('insuranceForm.step1.selectCountry')} error={getError("city_of_residence")}/> </div> {travellerFields.map((field, index) => { if (index === 0) return null; const firstNamePath = `travellers.${index}.first_name` as Path<InsuranceFormValues>; const lastNamePath = `travellers.${index}.last_name` as Path<InsuranceFormValues>; const birthdatePath = `travellers.${index}.birthdate` as Path<InsuranceFormValues>; return (<div key={field.id} className="mt-6 pt-6 border-t"><div className="flex justify-between items-center mb-3"><h3 className="text-lg font-semibold text-[#1A2C50]">{t(`insuranceForm.step1.additionalTraveller`)}</h3><Button type="button" variant="destructive" size="sm" onClick={() => removeTraveller(index)}>{t("insuranceForm.step1.remove")}</Button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><div className="flex items-start gap-2"><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactFirstName")} name={firstNamePath} register={register} placeholder={t("insuranceForm.step1.contactFirstName")} error={getError(firstNamePath)}/></div><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactLastName")} name={lastNamePath} register={register} placeholder={t("insuranceForm.step1.contactLastName")} error={getError(lastNamePath)}/></div></div></div><div className="w-full mt-1.5"><BirthDateField label={t('insuranceForm.step1.dob')} name={birthdatePath} control={control} getError={getError} watch={watch}/></div></div></div>); })} <Button type="button" variant="outline" onClick={() => appendTraveller({ first_name: "", last_name: "", birthdate: "", passport_number: "", passport_expiry_date: "" })} className="mt-6">{t("insuranceForm.step1.addAdditionalTraveller")}</Button> {getError("travellers" as Path<InsuranceFormValues>) && typeof getError("travellers" as Path<InsuranceFormValues>)?.message === 'string' && <p className="text-sm text-red-500 mt-1">{getError("travellers" as Path<InsuranceFormValues>)?.message}</p>} </>
//             )}
//             {/* STEP 2 UI */}
//             {step === 2 && ( <> <h2 className="text-2xl font-semibold mb-6 text-[#1A2C50]">{t("insuranceForm.step2.title")}</h2> <div className="mt-6"><SelectWithLabel label={t("insuranceForm.step2.tripPurpose")} name="trip_purpose" control={control} options={currentTripPurposes} placeholder={t("insuranceForm.step2.selectTripPurpose")} error={getError("trip_purpose")}/></div> <div className="mt-6"><InputWithLabel label={t("insuranceForm.step2.primaryCities")} name="primary_cities_regions_ukraine" register={register} error={getError("primary_cities_regions_ukraine")}/></div> <div className="mt-6"><InputWithLabel label={t("insuranceForm.step2.stayName")} name="stay_name" register={register} error={getError("stay_name")}/></div> <div className="mt-6"><InputWithLabel label={t("insuranceForm.step2.companyName")} name="company_name" register={register} error={getError("company_name")}/></div> <div className="mt-8 pt-6 border-t"> <h3 className="text-lg font-semibold mb-3 text-[#1A2C50]">{t("insuranceForm.step2.citiesVisitingTitle")}</h3> {cityFields.map((field, index) => { const cityNamePath = `trip_cities.${index}.name` as Path<InsuranceFormValues>; const cityZoneTypePath = `trip_cities.${index}.zoneType` as Path<InsuranceFormValues>; return (<div key={field.id} className="flex items-end gap-2 mb-3"><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step2.cityName").replace("{{index}}", (index + 1).toString())} name={cityNamePath} register={register} error={getError(cityNamePath)} placeholder={t("insuranceForm.step2.enterCityName")} /><input type="hidden" {...register(cityZoneTypePath)} value="GREEN"/></div><Button type="button" variant="destructive" size="sm" onClick={() => removeCity(index)}>{t("insuranceForm.step2.remove")}</Button></div>); })} <Button type="button" variant="outline" onClick={() => appendCity({ name: "", zoneType: "GREEN" })}>{t("insuranceForm.step2.addCity")}</Button> {getError("trip_cities" as Path<InsuranceFormValues>) && typeof getError("trip_cities" as Path<InsuranceFormValues>)?.message === 'string' && <p className="text-sm text-red-500 mt-1">{getError("trip_cities" as Path<InsuranceFormValues>)?.message}</p>} </div> </>
//             )}
//             {/* STEP 3 UI */}
//             {step === 3 && ( <> <h2 className="text-2xl font-semibold mb-6 text-[#1A2C50]">{t("insuranceForm.step3.title")}</h2> <div className="grid grid-cols-1 md:grid-cols-2 gap-6"> <div><div className="flex items-start gap-2"><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactFirstName")} name="emergency_contact_first_name" register={register} placeholder={t("insuranceForm.step1.contactFirstName")} error={getError("emergency_contact_first_name")}/></div><div className="flex-grow"><InputWithLabel label={t("insuranceForm.step1.contactLastName")} name="emergency_contact_last_name" register={register} placeholder={t("insuranceForm.step1.contactLastName")} error={getError("emergency_contact_last_name")}/></div></div></div> <div><p className="text-sm font-medium text-gray-900 mb-1 mt-1">{t("insuranceForm.step3.contactNumber")}</p><div className="flex items-start gap-2"><div className="w-1/3 shrink-0"><SelectWithLabel control={control} name="emergency_contact_phone_code" label="" options={currentCountryCodeOptions} placeholder={t("insuranceForm.step3.codePlaceholder")} error={getError("emergency_contact_phone_code")}/></div><div className="flex-grow"><InputWithLabel label="" name="emergency_contact_phone_number" type="tel" register={register} placeholder={t("insuranceForm.step3.numberPlaceholder")} error={getError("emergency_contact_phone_number")}/></div></div>{getError("emergency_contact_phone") && <p className="text-sm text-red-500 mt-1">{getError("emergency_contact_phone")?.message}</p>}</div> <InputWithLabel label={t("insuranceForm.step3.relationship")} name="emergency_contact_relation" register={register} error={getError("emergency_contact_relation")}/> </div> <h3 className="text-xl font-semibold mt-8 mb-4 text-[#1A2C50]">{t("insuranceForm.step3.optionalMedicalTitle")}</h3> <div className="space-y-4"> <div className="flex items-center space-x-2"><Controller name="has_medical_conditions" control={control} render={({ field }) => <Checkbox id="has_medical_conditions" checked={!!field.value} onCheckedChange={field.onChange}/>}/><Label htmlFor="has_medical_conditions">{t("insuranceForm.step3.preExistingConditions")}</Label></div> {watch("has_medical_conditions") && (<ControlledTextareaArray name="medical_conditions" control={control} label={t("insuranceForm.step3.listConditions")} error={getError("medical_conditions" as Path<InsuranceFormValues>)}/>)} <div className="flex items-center space-x-2"><Controller name="has_allergies" control={control} render={({ field }) => <Checkbox id="has_allergies" checked={!!field.value} onCheckedChange={field.onChange}/>}/><Label htmlFor="has_allergies">{t("insuranceForm.step3.allergies")}</Label></div> {watch("has_allergies") && (<ControlledTextareaArray name="allergies" control={control} label={t("insuranceForm.step3.listAllergies")} error={getError("allergies" as Path<InsuranceFormValues>)}/>)} <div className="flex items-center space-x-2"><Controller name="has_current_medications" control={control} render={({ field }) => <Checkbox id="has_current_medications" checked={!!field.value} onCheckedChange={field.onChange}/>}/><Label htmlFor="has_current_medications">{t("insuranceForm.step3.currentMedications")}</Label></div> {watch("has_current_medications") && (<ControlledTextareaArray name="current_medications" control={control} label={t("insuranceForm.step3.listMedications")} error={getError("current_medications" as Path<InsuranceFormValues>)}/>)} <InputWithLabel label={t("insuranceForm.step3.bloodType")} name="blood_type" register={register} error={getError("blood_type")}/> <TextareaWithLabel label={t("insuranceForm.step3.specialAssistance")} name="special_assistance" register={register} error={getError("special_assistance")}/> </div> </>
//             )}
//             {/* STEP 4 UI */}
//             {step === 4 && ( <> <h2 className="text-2xl font-semibold mb-6 text-[#1A2C50]">{t("insuranceForm.step4.summaryOfCoverage.title")}</h2> <div className="space-y-3 p-6 bg-gray-50 rounded-md border mb-6"> <div><strong>{t("insuranceForm.step4.summaryOfCoverage.travelDates")}</strong> {formatDateForDisplay(watchedValuesForSummary[0])} to {formatDateForDisplay(watchedValuesForSummary[1])}</div> <div><strong>{t("insuranceForm.step4.summaryOfCoverage.totalRiskZoneDays")}</strong> {calculatedTotalRiskZoneDays ?? "N/A"}</div> <div><strong>{t("insuranceForm.step4.summaryOfCoverage.riskZoneBreakdown")}</strong></div> <ul className="list-disc list-inside pl-4"> <li>{t("insuranceForm.step4.summaryOfCoverage.zone.green")}: {watch("green_zone_days") || 0}</li> <li>{t("insuranceForm.step4.summaryOfCoverage.zone.amber")}: {watch("amber_zone_days") || 0}</li> <li>{t("insuranceForm.step4.summaryOfCoverage.zone.red")}: {watch("red_zone_days") || 0} </li> {Number(watch("black_zone_days") || 0) > 0 && <li>{t("insuranceForm.step4.summaryOfCoverage.zone.black")}: {watch("black_zone_days")}</li>} </ul> <div><strong>{t("insuranceForm.step4.summaryOfCoverage.coverageSelected")}</strong></div> <ul className="list-disc list-inside pl-4"> <li>{t("insuranceForm.step4.summaryOfCoverage.coverage.medical")}: {getEmergencyMedicalLabel(watchedValuesForSummary[2])}</li> <li>{t("insuranceForm.step4.summaryOfCoverage.coverage.pa")}: {getPALabel(watchedValuesForSummary[3])}</li> <li>{t("insuranceForm.step4.summaryOfCoverage.coverage.transit")}: {watchedValuesForSummary[4] ? t("insuranceForm.yes250k") : t("insuranceForm.no")}</li> </ul> <div className="mt-4 pt-3 border-t"><strong className="text-xl">{t("insuranceForm.step4.summaryOfCoverage.totalQuote")}:</strong><span className="text-xl font-bold text-[#00BBD3] ml-2">{renderQuoteDisplay()}</span></div> {renderQuoteWarnings()} </div> <h3 className="text-xl font-semibold text-[#1A2C50] mb-3">{t("insuranceForm.step4.insuredDetails.title")}</h3> {watchedTravellersForSummary && watchedTravellersForSummary.map((traveller, index) => (<div key={`summary-traveller-${index}`} className="space-y-1 p-4 bg-gray-50 rounded-md border mb-3"><p className="font-semibold">{index === 0 ? t("insuranceForm.step4.insuredDetails.primaryTraveller") : `${t("insuranceForm.step4.insuredDetails.additionalTraveller")} ${index + 1}`}</p><div><strong>{t("insuranceForm.step4.insuredDetails.name")}:</strong> {formatFullName(traveller.first_name, traveller.last_name)}</div><div><strong>{t("insuranceForm.step4.insuredDetails.age")}:</strong> {calculateAge(traveller.birthdate) || "N/A"}</div>{index === 0 && <div><strong>{t("insuranceForm.step4.insuredDetails.nationality")}:</strong> {getNationalityLabel(watchedValuesForSummary[8])}</div>}</div>))} <h3 className="text-xl font-semibold text-[#1A2C50] mb-3">{t("insuranceForm.step4.tripInformation.title")}:</h3> <div className="space-y-1 p-4 bg-gray-50 rounded-md border mb-6"> <div><strong>{t("insuranceForm.step4.tripInformation.purpose")}:</strong> {getTripPurposeLabel(watchedValuesForSummary[9])}</div> <div><strong>{t("insuranceForm.step4.tripInformation.primaryRegions")}:</strong> {watchedValuesForSummary[10] || "N/A"}</div> {watchedTripCitiesForSummary && watchedTripCitiesForSummary.filter(city => city.name && city.name.trim() !== "").length > 0 && (<div><strong>{t("insuranceForm.step4.tripInformation.citiesVisiting")}:</strong><ul className="list-disc list-inside pl-4">{watchedTripCitiesForSummary.filter(city => city.name && city.name.trim() !== "").map((city, idx) => <li key={`summary-city-${idx}`}>{city.name}</li>)}</ul></div>)} </div> <h3 className="text-xl font-semibold text-[#1A2C50] mb-3">{t("insuranceForm.step4.emergencyContact.title")}:</h3> <div className="space-y-1 p-4 bg-gray-50 rounded-md border mb-6"><div><strong>{t("insuranceForm.step4.emergencyContact.name")}:</strong> {formatFullName(watchedValuesForSummary[11], watchedValuesForSummary[12])}</div><div><strong>{t("insuranceForm.step4.emergencyContact.number")}:</strong> {watchedValuesForSummary[13] || "N/A"}</div></div> <div className="mb-6"><InputWithLabel label={t("insuranceForm.step4.affiliateCode.label")} name="affiliate_code" register={register} error={getError("affiliate_code")}/></div> <div className="flex items-start space-x-3"> <Controller name="consent" control={control} render={({ field }) => (<Checkbox id="consent" checked={field.value === true} onCheckedChange={(checked) => field.onChange(checked === true ? true : undefined)} onBlur={field.onBlur}/>)}/> <div className="grid gap-1.5 leading-none"><Label htmlFor="consent" className="font-medium leading-snug">{t("insuranceForm.step4.consent.label")}</Label>{getError("consent") && <p className="text-sm text-red-500">{getError("consent")?.message}</p>}</div> </div> </>
//             )}
//             {/* Buttons */}
//             <div className="flex flex-col sm:flex-row justify-between pt-8 mt-8 border-t gap-4">
//               {step > 0 && (<Button type="button" variant="outline" onClick={handlePrevStep} className="w-full sm:w-auto px-8 py-3 text-base">{t("insuranceForm.actions.back")}</Button>)}
...
//   getCountryCodeOptions,
//   getNationalityOptions,
//   getCountryOptions, // This provides country of residence options
//   getCountryTravellingToOptions
// } from "@/lib/insuranceFormSchema";
// import {
//   InputWithLabel,
...
// const germanyGreenMERates: Partial<Record<GenerateQuotePayload['medical_coverage'], number>> = {
//   "25000": 19.68,
//   "50000": 22.95,
//   "100000": 27.64,
//   "150000": 32.25,
...
//     if (data.country_of_residence === "DE" && data.green_zone_days > 0 && germanyGreenMERates[medicalCoverageLevel] !== undefined) {
//         greenMERate = germanyGreenMERates[medicalCoverageLevel]!;
//         if (greenMERate <=0) {
...
//     if (!(data.country_of_residence === "DE" && data.green_zone_days > 0 && germanyGreenMERates[medicalCoverageLevel] !== undefined) && data.green_zone_days > 0 && (!medicalRates["greenME"] || medicalRates["greenME"] <= 0)) {
//     }
//     if (data.amber_zone_days > 0 && (!medicalRates["amberME"] || medicalRates["amberME"] <= 0)) {
//       warnings.push("Medical coverage for amber zone is not available or has zero cost for this coverage amount.");
//     }
...
//     const criticalErrorFields: (Path<InsuranceFormValues>)[] = ["trip_start_date", "trip_end_date", "green_zone_days", "amber_zone_days", "red_zone_days", "trip_countries", "emergency_medical_coverage", "personal_accident_coverage_level", "city_of_residence"];
//     const hasVisibleFormErrors = criticalErrorFields.some(field => get(formState.errors, field)) ||
//                                  (formState.errors.root && (formState.errors.root.message?.includes(zoneDaysSumErrorCheck) ||
//                                  formState.errors.root.message?.includes(coverageOrErrorCheck) ));
...
//       trip_start_date: startDate.toDate(), trip_end_date: endDate.toDate(),
//       trip_country: formValues.trip_countries[0] as string,
//       country_of_residence: formValues.city_of_residence as string, // Pass country of residence
//       travellers_count: (formValues.travellers?.length > 0 ? formValues.travellers.length : 1) as Integer,
//       medical_coverage: formValues.emergency_medical_coverage as GenerateQuotePayload['medical_coverage'],
//       pa_coverage: formValues.personal_accident_coverage_level as GenerateQuotePayload['pa_coverage'],
//       transit_coverage: formValues.add_transit_coverage,
...
//     let overallValidationPassed = true;
//     const partFieldsToValidate: Path<InsuranceFormValues>[] = [];
//     if (step === 1) partFieldsToValidate.push("c_phone_code", "c_phone_number", "c_whats_app_code", "c_whats_app_number", "c_passport_number", "c_passport_expiry_date", "city_of_residence"); // Added city_of_residence for validation
//     if (step === 3) partFieldsToValidate.push("emergency_contact_phone_code", "emergency_contact_phone_number");
...
//                   <TooltipProvider>
//                     <div className="flex items-center space-x-2">
//                       <Controller name="add_transit_coverage" control={control} render={({ field }) => (<Checkbox id="add_transit_coverage" checked={field.value} onCheckedChange={field.onChange} disabled={coverageDisabled || !watch("city_of_residence")}/>)}/>
//                       <Tooltip>
//                         <TooltipTrigger asChild><Label htmlFor="add_transit_coverage" className={(coverageDisabled || !watch("city_of_residence")) ? "text-gray-400" : ""}>{t("insuranceForm.addTransitCover")} <strong>€25</strong></Label></TooltipTrigger>
//                         <TooltipContent side="bottom" align="start" sideOffset={4}><div className="relative"><div className="bg-muted px-3 py-2 text-sm shadow">{t("insuranceForm.addTransitCoverTooltip")}</div><div className="absolute top-0 left-2.5 -translate-y-full w-0 h-0 border-l-8 border-r-8 border-b-8 border-l-transparent border-r-transparent border-b-muted"/></div></TooltipContent>
//                       </Tooltip>
//                     </div>
//                   </TooltipProvider>
//                 </div>
//                 <Button type="button" onClick={() => setShowQuote(true)} className="mt-6 w-full" disabled={!watch("city_of_residence")}>{t("insuranceForm.actions.getQuote")}</Button>
//                 {showQuote && (
//                   <div className="mt-8 p-6 bg-gray-50 rounded-md border">
//                     <h3 className="text-xl font-semibold text-[#1A2C50] mb-2">{t("insuranceForm.quoteSummary")}</h3>
//                     <p className="text-2xl font-bold text-[#00BBD3]">{renderQuoteDisplay()}</p>
//                     {renderQuoteWarnings()}
//                     {quoteResult?.ok && quoteResult?.data && (
//                       <div className="text-sm mt-2">
//                         <p>{t("insuranceForm.medical")}: {getEmergencyMedicalLabel(watch("emergency_medical_coverage"))} ({formatEuro(quoteResult.data.medicalCoverAmount)})</p>
//                         <p>{t("insuranceForm.pa")}: {getPALabel(watch("personal_accident_coverage_level"))} ({formatEuro(quoteResult.data.paCoverAmount)})</p>
//                         <p>{t("insuranceForm.transit")}: {watch("add_transit_coverage") ? formatEuro(pricing.transitCost) : t("insuranceForm.no")}</p>
//                       </div>
//                     )}
//                   </div>
...
//                   <div><strong>{t("insuranceForm.step4.summaryOfCoverage.coverageSelected")}</strong></div>
//                   <ul className="list-disc list-inside pl-4">
//                     <li>{t("insuranceForm.step4.summaryOfCoverage.coverage.medical")}: {getEmergencyMedicalLabel(watchedValuesForSummary[2])} {quoteResult?.data ? `(${formatEuro(quoteResult.data.medicalCoverAmount)})` : ''}</li>
//                     <li>{t("insuranceForm.step4.summaryOfCoverage.coverage.pa")}: {getPALabel(watchedValuesForSummary[3])} {quoteResult?.data ? `(${formatEuro(quoteResult.data.paCoverAmount)})` : ''}</li>
//                      <li>{t("insuranceForm.step4.summaryOfCoverage.coverage.transit")}: {watchedValuesForSummary[4] ? t("insuranceForm.yes250k") : t("insuranceForm.no")}</li>
//                   </ul>
//                   <div className="mt-4 pt-3 border-t"><strong className="text-xl">{t("insuranceForm.step4.summaryOfCoverage.totalQuote")}:</strong><span className="text-xl font-bold text-[#00BBD3] ml-2">{renderQuoteDisplay()}</span></div>
//                   {renderQuoteWarnings()}
...
//     if (!quoteResult || !quoteResult.ok || !quoteResult.data) {
//       alert("Quote is not available or contains errors. Cannot proceed to payment.");
...
//     if (paymentAmount <= 0) {
//         alert("Payment amount is invalid (must be greater than 0). Cannot proceed.");
//         return;
//     }

  
//     if (typeof window === "undefined") {
//         alert("Cannot initiate payment: Window object not found.");
//         return;
//     }
//     const flywireCheckoutUrl = new URL('/flywire-checkout', window.location.origin);
//     flywireCheckoutUrl.searchParams.append('firstName', payerFirstName || 'Payer');
//     flywireCheckoutUrl.searchParams.append('lastName', payerLastName || 'Name');
//     flywireCheckoutUrl.searchParams.append('email', payerEmail || 'email@example.com');
//     flywireCheckoutUrl.searchParams.append('amount', paymentAmount.toString());
//     flywireCheckoutUrl.searchParams.append('invoiceNumber', generatedInvoiceNumber);

//     window.open(flywireCheckoutUrl.toString(), '_blank');

//     alert("Payment process initiated in a new tab. Please complete your payment there.");
